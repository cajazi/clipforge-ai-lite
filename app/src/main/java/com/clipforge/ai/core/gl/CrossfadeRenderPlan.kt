...
object CrossfadeRenderPlan {
    ...
    // Bounce transition: clip B pops in with an elastic overshoot (zoom-class, single variant).
    private val BOUNCE_TYPES = setOf("BOUNCE")
    ...
    sealed class Op {
        ...
        /** Full-screen flash burst hides a midpoint A/B swap over the overlap window. */
        data class Flash(
            val pathA: String, val aTailStartMs: Long, val aEndMs: Long,
        ...
        /** Bounce clip B into place over a static clip A with an elastic overshoot. */
        data class Bounce(
            val pathA: String, val aTailStartMs: Long, val aEndMs: Long,
            val pathB: String, val bHeadStartMs: Long, val durationMs: Long
        ) : Op()
        ...
        /** Directional B-under-A reveal with a soft wipe edge. */
        data class Wipe(
            val pathA: String, val aTailStartMs: Long, val aEndMs: Long,
            val pathB: String, val bHeadStartMs: Long, val durationMs: Long,
            val direction: String
    ...
    suspend fun build(context: Context, projectId: String): List<Op> =
        withContext(Dispatchers.IO) {
            ...
            val isFlash = BooleanArray(entries.size)
            ...
            val isBounce = BooleanArray(entries.size)
            val bounceDurArr = LongArray(entries.size)
            ...
            val isWipe = BooleanArray(entries.size)
            val wipeDurArr = LongArray(entries.size)
            val wipeDirArr = arrayOfNulls<String>(entries.size)
            ...
            for (i in 0 until entries.size - 1) {
                ...
                } else if (t in FLASH_TYPES && durMs > 0L) {
                    isFlash[i] = true
                    flashTypeArr[i] = t
                ...
                } else if (t in BOUNCE_TYPES && durMs > 0L) {
                    isBounce[i] = true
                    bounceDurArr[i] = durMs
                    Log.d(TAG, "boundary $i->${i + 1} plan=BOUNCE requestedMs=$durMs")
                ...
                } else if (t in WIPE_TYPES && durMs > 0L) {
                    isWipe[i] = true
                    wipeDirArr[i] = t
                    wipeDurArr[i] = durMs
                    Log.d(TAG, "boundary $i->${i + 1} plan=WIPE requestedMs=$durMs direction=$t")
            ...
            fun setBoundaryConsumption(i: Int, consumptionMs: Long) {
                when {
                    ...
                    isBounce[i] -> bounceDurArr[i] = consumptionMs
            ...
            for (i in entries.indices) {
                ...
                // Flash: hard A/B swap hidden under a full-screen color burst (overlap-style).
                if (i < entries.size - 1 && isFlash[i]) {
                    val flashMs = flashDurArr[i]
                    val next = entries[i + 1]
                    if (flashMs > 0L) {
                        ops.add(
                            Op.Flash(
                                pathA = e.path,
                                aTailStartMs = clipEnd - flashMs,
                                aEndMs = clipEnd,
                                pathB = next.path,
                                bHeadStartMs = next.trimStartMs,
                                durationMs = flashMs,
                ...
                // Bounce: clip B pops in over A's tail with an elastic overshoot (overlap-style).
                if (i < entries.size - 1 && isBounce[i]) {
                    val bMs = bounceDurArr[i]
                    val next = entries[i + 1]
                    if (bMs > 0L) {
                        ops.add(
                            Op.Bounce(
                                pathA = e.path,
                                aTailStartMs = clipEnd - bMs,
                                aEndMs = clipEnd,
                                pathB = next.path,
                                bHeadStartMs = next.trimStartMs,
                                durationMs = bMs
                ...
                // Wipe: B is revealed under A through a soft directional edge (overlap-style).
                if (i < entries.size - 1 && isWipe[i]) {
                    val wMs = wipeDurArr[i]
                    val next = entries[i + 1]
                    if (wMs > 0L) {
                        ops.add(
                            Op.Wipe(
            ...
            ops.forEachIndexed { idx, op ->
                when (op) {
                    ...
                    is Op.Bounce -> Log.d(TAG, "[$idx] BOUNCE ${op.durationMs}ms  A=${op.pathA.substringAfterLast('/')}[${op.aTailStartMs}..${op.aEndMs}]  B=${op.pathB.substringAfterLast('/')}[head ${op.bHeadStartMs}]")
    ...
    private fun flashColorFor(type: String?): Int = when (type?.uppercase()) {
        "FLASH_BLACK" -> android.graphics.Color.BLACK
        "FLASH_WARM" -> android.graphics.Color.rgb(255, 214, 128)
        "FLASH_BLUE" -> android.graphics.Color.rgb(120, 190, 255)
